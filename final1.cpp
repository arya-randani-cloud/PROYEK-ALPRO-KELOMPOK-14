...
// ============================================================
//   SISTEM LOGIN TERPUSAT CERAN_HUB
//   Satu pintu masuk (satu form username & password) yang secara
//   otomatis mengenali jenis akun (USER / ADMIN / TOKO) dan
//   mengarahkan ke TAMPILAN yang berbeda untuk masing-masing role.
//   Tidak ada lagi menu pilih-modul manual seperti sebelumnya:
//   tampilan yang muncul murni ditentukan oleh akun yang login.
// ============================================================

// Enum penanda jenis/role akun hasil login
enum RolePenggunaUtama {
    ROLE_USER_PELANGGAN,   // -> tampilan belanja pelanggan
    ROLE_ADMIN_MALL,       // -> tampilan dashboard admin mall
    ROLE_ADMIN_TOKO,       // -> tampilan admin/vendor toko
    ROLE_TIDAK_DIKENALI    // -> gagal login / akun tidak ditemukan
};

// Struktur satu akun pada database login terpusat
struct AkunLoginTerpusat {
    string username;
    string password;
    RolePenggunaUtama role;
    string namaTampilan;
};

// Mencetak header pembuka gerbang login (murni kosmetik terminal)
void cetakHeaderGerbangLogin() {
    cout << "\n=========================================\n";
    cout << "     CERAN_HUB - GERBANG LOGIN TERPUSAT   \n";
    cout << "     Proyek ALPRO Kelompok 14              \n";
    cout << "=========================================\n";
    cout << "Tampilan yang muncul akan otomatis menyesuaikan\n";
    cout << "jenis akun Anda setelah login berhasil:\n";
    cout << "  - Akun USER  -> Tampilan Belanja Pelanggan\n";
    cout << "  - Akun ADMIN -> Tampilan Dashboard Admin Mall\n";
    cout << "  - Akun TOKO  -> Tampilan Admin/Vendor Toko\n";
    cout << "=========================================\n";
}

// Mengisi database akun login terpusat.
// Kredensial berikut menggabungkan akun yang sebelumnya sudah
// dipakai pada masing-masing modul (Pelanggan, Admin Mall, Toko)
// sehingga tidak ada data baru yang mengubah alur modul asli.
int isiDatabaseAkunLoginTerpusat(AkunLoginTerpusat daftarAkun[]) {
    int n = 0;

    // --- Akun bertipe USER (Pelanggan CeranMall) ---
    daftarAkun[n++] = {"arya123", "jogja2026", ROLE_USER_PELANGGAN, "Arya Randani"};
    daftarAkun[n++] = {"user2",   "pass2",     ROLE_USER_PELANGGAN, "Budi Santoso"};

    // --- Akun bertipe ADMIN (Admin Mall CeranMall) ---
    daftarAkun[n++] = {"admin01", "ceran2026admin", ROLE_ADMIN_MALL, "Admin Mall CeranHub"};

    // --- Akun bertipe TOKO (Admin/Vendor Toko) ---
    daftarAkun[n++] = {"admin",  "admin123", ROLE_ADMIN_TOKO, "Admin Toko"};
    daftarAkun[n++] = {"toko01", "toko2026", ROLE_ADMIN_TOKO, "Pemilik Toko"};

    return n;
}

// Memproses input login dan mengembalikan role akun yang berhasil login.
// namaUserOut akan diisi dengan nama tampilan akun yang login.
RolePenggunaUtama prosesLoginTerpusat(string &namaUserOut) {
    const int MAKS_AKUN = 10;
    AkunLoginTerpusat daftarAkun[MAKS_AKUN];
    int totalAkun = isiDatabaseAkunLoginTerpusat(daftarAkun);

    const int MAKS_PERCOBAAN = 3;
    int percobaan = 0;

    cetakHeaderGerbangLogin();
    ...
    while (percobaan < MAKS_PERCOBAAN) {
        ...
        for (int i = 0; i < totalAkun; i++) {
            if (daftarAkun[i].username == inputUser && daftarAkun[i].password == inputPass) {
                namaUserOut = daftarAkun[i].namaTampilan;
                cout << "\n[LOGIN SUKSES] Selamat datang, " << namaUserOut << "!\n";
                return daftarAkun[i].role;
            }
        }

        percobaan++;
        cout << "\n[GAGAL] Username atau Password salah. Percobaan ("
             << percobaan << "/" << MAKS_PERCOBAAN << ")\n";
    }

    cout << "\nBatas percobaan login terlampaui. Program dihentikan.\n";
    return ROLE_TIDAK_DIKENALI;
}

// ---------------------------------------------------------------
// TAMPILAN UNTUK ROLE USER (PELANGGAN)
// Setelah login sebagai user, pelanggan hanya melihat menu-menu
// yang relevan dengan belanja: berbelanja penuh di CeranMall atau
// memakai sistem pemesanan cepat.
// ---------------------------------------------------------------
void tampilanUntukRoleUser(const string &namaPengguna) {
    int subPilihan;
    do {
        cout << "\n=========================================\n";
        cout << "   TAMPILAN USER/PELANGGAN - " << namaPengguna << "\n";
        cout << "=========================================\n";
        cout << "1. Belanja di CeranMall (Katalog Lengkap)\n";
        cout << "2. Sistem Pemesanan Cepat\n";
        cout << "0. Logout\n";
        cout << "=========================================\n";
        cout << "Pilih menu: ";
        cin >> subPilihan;
        ...
        switch (subPilihan) {
            case 1: modPelanggan::jalankanPelanggan(); break;
            case 2: modPemesanan::jalankanPemesanan(); break;
            case 0: cout << "\nLogout dari tampilan User. Sampai jumpa, " << namaPengguna << "!\n"; break;
            default: cout << "\nPilihan tidak valid!\n";
        }
    } while (subPilihan != 0);
}
...
// ---------------------------------------------------------------
// TAMPILAN UNTUK ROLE ADMIN (ADMIN MALL)
// Setelah login sebagai admin mall, langsung diarahkan ke dashboard
// pengelolaan mall (vendor, pelanggan, produk, pendapatan, dll)
// tanpa melalui menu pemilihan modul manual.
// ---------------------------------------------------------------
void tampilanUntukRoleAdmin(const string &namaPengguna) {
    cout << "\n=========================================\n";
    cout << "   TAMPILAN ADMIN MALL - " << namaPengguna << "\n";
    cout << "=========================================\n";
    modVendor::menuAdminMall();
    cout << "\nLogout dari tampilan Admin Mall. Sampai jumpa, " << namaPengguna << "!\n";
}

// ---------------------------------------------------------------
// TAMPILAN UNTUK ROLE TOKO (ADMIN/VENDOR TOKO)
// Setelah login sebagai akun toko, tampilan yang muncul adalah
// menu khusus pengelolaan toko: registrasi toko baru atau membuka
// dashboard toko lengkap (final) sesuai kebutuhan vendor.
// ---------------------------------------------------------------
void tampilanUntukRoleToko(const string &namaPengguna) {
    int subPilihan;
    do {
        cout << "\n=========================================\n";
        cout << "   TAMPILAN ADMIN TOKO - " << namaPengguna << "\n";
        cout << "=========================================\n";
        cout << "1. Registrasi Toko Baru\n";
        cout << "2. Dashboard Toko Lengkap (Final)\n";
        cout << "0. Logout\n";
        cout << "=========================================\n";
        cout << "Pilih menu: ";
        cin >> subPilihan;
        ...
        switch (subPilihan) {
            case 1: modAddAdmin::jalankanAddAdminToko(); break;
            case 2: modAdminFinal::jalankanAdminTokoFinal(); break;
            case 0: cout << "\nLogout dari tampilan Admin Toko. Sampai jumpa, " << namaPengguna << "!\n"; break;
            default: cout << "\nPilihan tidak valid!\n";
        }
    } while (subPilihan != 0);
}
...
// ============================================================
//   MAIN CERAN_HUB
//   Alur: login sekali di gerbang terpusat -> sistem mengenali
//   role dari kombinasi username & password -> tampilan yang
//   ditampilkan otomatis berbeda antara USER, ADMIN, dan TOKO.
// ============================================================
int main() {
    string namaPengguna;
    RolePenggunaUtama role = prosesLoginTerpusat(namaPengguna);

    switch (role) {
        case ROLE_USER_PELANGGAN:
            tampilanUntukRoleUser(namaPengguna);
            break;
        case ROLE_ADMIN_MALL:
            tampilanUntukRoleAdmin(namaPengguna);
            break;
        case ROLE_ADMIN_TOKO:
            tampilanUntukRoleToko(namaPengguna);
            break;
        case ROLE_TIDAK_DIKENALI:
        default:
            // Login gagal setelah batas percobaan, program berhenti.
            break;
    }
